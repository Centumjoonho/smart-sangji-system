...
from .angle_calculaters import calculate_angle
import os
from project.settings import BASE_DIR, KO_FONT_URL
...
def measure_arm_distance(frame, win_name, success_crit=30):
    ...
    draw.text( (10,20), "양 팔을 30도 미만으로 구부려주세요", font=ImageFont.truetype(KO_FONT_URL,font_size), fill=(0,0,255))
    ...
    try:    
        ...
        img_pil = Image.fromarray(frame)
        draw = ImageDraw.Draw(img_pil)
        draw.text( (10,60), '왼쪽 팔 각도:' + str(int(angle['left'])) + '   Dist: {0:2.1f}'.format(distance['left']),
        font=ImageFont.truetype(KO_FONT_URL,font_size), fill=(0,0,255))
        frame = np.array(img_pil)
        ...
        img_pil = Image.fromarray(frame)
        draw = ImageDraw.Draw(img_pil)
        draw.text( (10,100), '오른 팔 각도:' + str(int(angle['right'])) + '   Dist: {0:2.1f}'.format(distance['right']),
        font=ImageFont.truetype(KO_FONT_URL,font_size), fill=(0,0,255))
        frame = np.array(img_pil)
