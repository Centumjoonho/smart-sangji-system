...
from project.settings import MOLE_IMAGE_URL
import os
...
class MoleManager():
    ...
    def __init__(self, bg_screen_size, divide_unit) -> None:
        ...
        self.win_name = 'Mole'
        self.bg_frame =  cv2.imread(os.path.join(MOLE_IMAGE_URL,'bg_yellow.png'), cv2.IMREAD_COLOR)
        self.mole_list = None
        self.mole_img = cv2.imread(os.path.join(MOLE_IMAGE_URL,'mole_jklee.jpg'), cv2.IMREAD_COLOR)
        self.start_img = cv2.imread(os.path.join(MOLE_IMAGE_URL,'start_bg.png'), cv2.IMREAD_COLOR)
        self.mission_completed_img = cv2.imread(os.path.join(MOLE_IMAGE_URL,'bg_mission_completed.png'), cv2.IMREAD_COLOR)
        self.show_up_time = 1
